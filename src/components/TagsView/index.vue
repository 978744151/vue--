...
import visitedList from "@/router/modules/remaining";
...
const visited = computed(() => useTagsViewStore().visitedViews);
const iframeViews = computed(() => useTagsViewStore().iframeViews);
const cachedViews = computed(() => useTagsViewStore().cachedViews);
const visitedViews = computed(() =>[...visited.value,...iframeViews.value]);
// console.log(visitedViews)
// const routes = computed(() => usePermissionStore().routes);
// const routes = iframeList;
const routes = []
const theme = [];
...
function handleLink(tag) {
  // console.log(tag)
  useTagsViewStore().addView(tag)
  router.push({
    path: tag.path,
    query:tag.query,
...
function addTags() {
  ...
  if (name) {
    ...
    const views = iframeList.find(e=>route.path === e.path)
    useTagsViewStore().addIframeView(views);
  }
  return false;
}
...
function moveToCurrentTags(visitedViews) {
  nextTick(() => {
